...
public partial class AutDashboard : System.Web.UI.Page
{
    ...
    public void fillATT()
    {
       ...
       // DateTime date = DateTime.Parse("01-Feb-2016");
        ...
        DataSet emp = daM.selectEmpIDUser(Session["userId"].ToString());
        int empId = Int32.Parse(emp.Tables[0].Rows[0][0].ToString());
        DataSet dsFP = daM.FP(empId);
        try
        {
            if (dsFP.Tables[0].Rows[0][0].ToString() == "NO")
            {
               // lblMSG.Text = " Attendance not required for this user  ";
                    ...
                    for (int i = 0; i < dsSCH.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (dsSCH.Tables[0].Rows[i][6].ToString() == "Or" && dsSCH.Tables[0].Rows[i][9].ToString() == "Yes")
                        {
                            if (dsTime.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (dayN == "Saturday" && dateT.ToString() == "13:00:00")
                                {
                                    dateT = DateTime.Parse("12:00:00");
    ...
    public void fillData()
    {
        ...
        DataSet emp = daM.selectEmpIDUser(Session["userId"].ToString());
        int empId = Int32.Parse(emp.Tables[0].Rows[0][0].ToString());
        DataSet ds = DA.UaLeaveTotal(empId, "Unauthorized");
        DataSet dsRV = DA.UaLeaveTotal(empId, "Reversal Unauthorized");
        if (ds.Tables[0].Rows.Count ==0)
        {
            maxDAte.Text = "0";
        }
        ...
        {
            maxDAte.Text = ds.Tables[0].Rows[0][0].ToString();
        }
        if (dsRV.Tables[0].Rows.Count == 0)
        {
            maxTime.Text = "0";
        }
        else
        {
            maxTime.Text = dsRV.Tables[0].Rows[0][0].ToString();
        }
